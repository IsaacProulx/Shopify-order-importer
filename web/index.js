...
app.get("/api/draftOrders/count", async (_req, res) => {
  const countData = await shopify.api.rest.DraftOrder.count({
    session: res.locals.shopify.session,
  });
  res.status(200).send(countData);
})
...
app.post("/api/orders/create", async (req, res) => {
  ...
  console.log(data);
  try {
    await orderCreator(res.locals.shopify.session,data.lineItems);
  } catch (e) {
